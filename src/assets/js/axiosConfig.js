...
Api.prototype = {
  ...
  getBlogInfosByPage: async function(limt, currentPage) {
    const ret = await axios.get('/api/blog-info?_sort=time&_order=DESC&_limit=' + limt + '&_page=' + (currentPage + 1))
  ...
  getBlogInfosByIDs: async function(blogInfoIDs) {
    ...
    const ret = await axios.get('/api/blog-info?_sort=time&_order=DESC&' + parametersString)
    fixBlogInfos(ret.data)
    return ret
  },
  ...
  getComment: async function(blogID, limt, page) {
    const response = await axios.get('/api/comment?blogID=' + blogID + '&_limit=' + limt + '&_page=' + page + '&_sort=time')
    const rootTotalCount = parseInt(response.headers['x-total-count'])
    const comments = response.data.slice()
  ...
  getBloggerInfo: async function() {
    const response = await axios.get('/api/blogger-info')
    return response
  }
}
